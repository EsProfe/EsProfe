"use strict";
(function(){
const SESSION_KEY="esprofe_free_session_v1";
const labels={
...
;
function blank(){return{answers:0,correct:0,streak:0,bestStreak:0}}
function load(){try{const x=JSON.parse(sessionStorage.getItem(SESSION_KEY));return{answers:Number(x?.answers)||0,correct:Number(x?.correct)||0,streak:Number(x?.streak)||0,bestStreak:Number(x?.bestStreak)||0}}catch(_){return blank()}}
let state=load();
let enabled=false;
