...
function applyTopicCardTranslations() {
    document.querySelectorAll(".topic-card").forEach((card) => {
        ...
        if (title && translations[titleKey]) title.textContent = translations[titleKey];
        if (hint && translations[hintKey]) hint.textContent = translations[hintKey];
    });
    const topicLabel = document.getElementById("topicLabel");
    if (topicLabel && translations.topicLabel) topicLabel.textContent = translations.topicLabel;
    const topicHint = document.getElementById("topicHint");
    if (topicHint && translations.topicHint) topicHint.textContent = translations.topicHint;
}

function applyTrainerTranslations() {
    const map = {
        trainerHeading: "trainerHeading",
        sourceVerbLabel: "sourceVerbLabel",
        requestedFormLabel: "requestedFormLabel",
        testPersonLabel: "testPersonLabel"
    };
...
async function loadLanguage(lang) {
    try {
        ...
        if (testPercent && testPercent.dataset.percent && translations.result) testPercent.textContent = `${translations.result} ${testPercent.dataset.percent}%`;

        const result = document.getElementById("result");
        if (result && result.dataset.type === "correct" && translations.correct) result.textContent = `✅ ${translations.correct}`;
        if (result && result.dataset.type === "incorrect") result.textContent = `❌ ${translations.incorrect} ${translations.correctAnswer} ${result.dataset.answer}`;
        const testResult = document.getElementById("testResult");
        if (testResult && testResult.dataset.type === "correct" && translations.correct) testResult.textContent = `✅ ${translations.correct}`;
        if (testResult && testResult.dataset.type === "incorrect") testResult.textContent = `❌ ${translations.incorrect} ${translations.correctAnswer} ${testResult.dataset.answer}`;

        applyTopicCardTranslations();
        applyTrainerTranslations();
