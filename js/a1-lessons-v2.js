...
(function () {
  ...
  function cityVisualHTML() {
    ...
    if (lesson?.id === "home") { const cards=lesson.flashcards||[], n={ru:1,uk:2,en:3,es:4}[lang()]||1; return `<section class="a1-vocab-cards"><h3>🧠 ${esc(({ru:"Карточки: нажми и вспомни перевод",uk:"Картки: натисни й згадай переклад",en:"Flashcards: tap and recall the translation",es:"Tarjetas: pulsa y recuerda la traducción"})[lang()]||"")}</h3><div>${cards.map(([word,...rest])=>`<button type="button" class="a1-vocab-card" data-card><span>${rest[4]}</span><b>${esc(word)}</b><small>${esc(rest[n-1])}</small></button>`).join("")}</div></section>`; }
