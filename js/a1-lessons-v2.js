...
(function () {
  ...
  function cityVisualHTML() {
    if (["family","home"].includes(lesson?.id)) { const cards=lesson.id==="family"?[["👨","padre"],["👩","madre"],["👦","hermano"],["👧","hermana"]]:[["🍳","cocina"],["🛁","baño"],["🛏️","dormitorio"],["🛋️","salón"]]; const title={ru:lesson.id==="family"?"Семейное дерево":"Мой дом",uk:lesson.id==="family"?"Родинне дерево":"Мій дім",en:lesson.id==="family"?"Family tree":"My home",es:lesson.id==="family"?"Árbol familiar":"Mi casa"}[lang()]||""; return `<section class="a1-vocab-visual"><h3>${esc(title)}</h3><div>${cards.map(([icon,word])=>`<article><span>${icon}</span><b>${word}</b></article>`).join("")}</div></section>`; }
