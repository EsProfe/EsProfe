...
(function () {
  ...
  function tenerExpressionVisualHTML() {
    ...
    const moreTitle = { ru: "Другие частые выражения", uk: "Інші часті вирази", en: "Other common expressions", es: "Otras expresiones frecuentes" }[lang()] || "Otras expresiones frecuentes";
    const extra = {
      ru: [["🩹", "tener dolor de + часть тела", "испытывать боль"], ["🤧", "tener gripe", "болеть гриппом"], ["📌", "tener necesidad de + nombre / infinitivo", "нуждаться в чём-либо"], ["🧘", "tener paciencia", "быть терпеливым"], ["😨", "tener miedo de + infinitivo / a + nombre", "бояться"], ["⏱️", "tener prisa", "торопиться"], ["💚", "tener celos de", "ревновать"], ["🤝", "tener confianza en", "доверять"], ["😳", "tener vergüenza de", "стыдиться"], ["✅", "tener razón", "быть правым"], ["⚖️", "tener derecho a", "иметь право на"], ["⚠️", "tener cuidado con", "быть осторожным с"], ["🏆", "tener éxito", "добиться успеха"], ["🙋", "tener la culpa", "быть виноватым"], ["🍀", "tener suerte", "иметь удачу"], ["📝", "tener en cuenta", "принимать во внимание"], ["📍", "tener lugar", "происходить, иметь место"]],
      uk: [["🩹", "tener dolor de + parte del cuerpo", "відчувати біль"], ["🤧", "tener gripe", "хворіти на грип"], ["📌", "tener necesidad de + nombre / infinitivo", "потребувати чогось"], ["🧘", "tener paciencia", "бути терплячим"], ["😨", "tener miedo de + infinitivo / a + nombre", "боятися"], ["⏱️", "tener prisa", "поспішати"], ["💚", "tener celos de", "ревнувати"], ["🤝", "tener confianza en", "довіряти"], ["😳", "tener vergüenza de", "соромитися"], ["✅", "tener razón", "мати рацію"], ["⚖️", "tener derecho a", "мати право на"], ["⚠️", "tener cuidado con", "бути обережним з"], ["🏆", "tener éxito", "досягати успіху"], ["🙋", "tener la culpa", "бути винним"], ["🍀", "tener suerte", "мати удачу"], ["📝", "tener en cuenta", "брати до уваги"], ["📍", "tener lugar", "відбуватися, мати місце"]],
      en: [["🩹", "tener dolor de + body part", "feel pain"], ["🤧", "tener gripe", "have the flu"], ["📌", "tener necesidad de + noun / infinitive", "need something"], ["🧘", "tener paciencia", "be patient"], ["😨", "tener miedo de + infinitive / a + noun", "be afraid of"], ["⏱️", "tener prisa", "be in a hurry"], ["💚", "tener celos de", "be jealous of"], ["🤝", "tener confianza en", "trust"], ["😳", "tener vergüenza de", "be ashamed of"], ["✅", "tener razón", "be right"], ["⚖️", "tener derecho a", "have the right to"], ["⚠️", "tener cuidado con", "be careful with"], ["🏆", "tener éxito", "be successful"], ["🙋", "tener la culpa", "be to blame"], ["🍀", "tener suerte", "be lucky"], ["📝", "tener en cuenta", "take into account"], ["📍", "tener lugar", "take place"]],
      es: [["🩹", "tener dolor de + parte del cuerpo", "sentir dolor"], ["🤧", "tener gripe", "estar enfermo de gripe"], ["📌", "tener necesidad de + nombre / infinitivo", "necesitar algo"], ["🧘", "tener paciencia", "ser paciente"], ["😨", "tener miedo de + infinitivo / a + nombre", "sentir miedo"], ["⏱️", "tener prisa", "ir con prisa"], ["💚", "tener celos de", "estar celoso"], ["🤝", "tener confianza en", "confiar"], ["😳", "tener vergüenza de", "avergonzarse"], ["✅", "tener razón", "estar en lo cierto"], ["⚖️", "tener derecho a", "tener derecho"], ["⚠️", "tener cuidado con", "ir con cuidado"], ["🏆", "tener éxito", "triunfar"], ["🙋", "tener la culpa", "ser culpable"], ["🍀", "tener suerte", "ser afortunado"], ["📝", "tener en cuenta", "considerar"], ["📍", "tener lugar", "suceder"]],
    }[lang()] || [];
    return `<section class="a1-tener-visual" aria-label="${esc(t[0])}"><h3>${esc(t[0])}</h3><p>${esc(t[1])}</p><img class="a1-tener-illustration" src="data/images/tener-expresiones.png" alt="TENER + expresiones: tener hambre, sed, sueño, calor, frío y años"><div class="a1-tener-grid">${cards.map(([icon, phrase, meaning]) => `<article><span aria-hidden="true">${icon}</span><b>${esc(phrase)}</b><small>${esc(meaning)}</small></article>`).join("")}</div><h4>${esc(moreTitle)}</h4><div class="a1-tener-extra">${[extra.slice(0, 9), extra.slice(9)].map((group) => `<section>${group.map(([icon, phrase, meaning]) => `<div><span aria-hidden="true">${icon}</span><b>${esc(phrase)}</b><small>${esc(meaning)}</small></div>`).join("")}</section>`).join("")}</div></section>`;
  }
  ...
  function cityVisualHTML() {
    ...
    if (lesson?.flashcards) { const cards=lesson.flashcards||[], n={ru:1,uk:2,en:3,es:4}[lang()]||1,active=lesson.activeRecall,stair='<svg width="30" height="30" viewBox="0 0 48 48" aria-label="stairs"><path d="M5 40h10V30h10V20h10V10h8" fill="none" stroke="#1b55c9" stroke-width="5"/></svg>',pass='<svg width="32" height="30" viewBox="0 0 48 48" aria-label="travel pass"><rect x="6" y="10" width="36" height="28" rx="4" fill="#e7f0ff" stroke="#1b55c9" stroke-width="2"/><path d="M11 19h26M11 25h16M11 31h11" stroke="#1b55c9" stroke-width="2"/><circle cx="35" cy="29" r="4" fill="#4ba86c"/></svg>',coins='<svg width="30" height="30" viewBox="0 0 48 48" aria-label="coins"><ellipse cx="23" cy="33" rx="15" ry="7" fill="#f6c344" stroke="#b77a05" stroke-width="2"/><path d="M8 27v6c0 4 30 5 30 0v-6" fill="#ffd968" stroke="#b77a05" stroke-width="2"/><ellipse cx="23" cy="27" rx="15" ry="7" fill="#ffe388" stroke="#b77a05" stroke-width="2"/><path d="M23 23v8m-3-5h6" stroke="#b77a05" stroke-width="2" stroke-linecap="round"/></svg>',title=active?({ru:"Слова для активного запоминания",uk:"Слова для активного запам’ятовування",en:"Words for active recall",es:"Palabras para el recuerdo activo"}[lang()]||""):({ru:"Карточки: нажми и вспомни перевод",uk:"Картки: натисни й згадай переклад",en:"Flashcards: tap and recall the translation",es:"Tarjetas: pulsa y recuerda la traducción"})[lang()]; return `<section class="a1-vocab-cards"><h3>🧠 ${esc(title||"")}</h3><div>${cards.map(([word,...rest])=>`<button type="button" class="a1-vocab-card" data-card><span>${word==="la escalera"?stair:word==="el cambio"?coins:word==="el abono de transporte"?pass:rest[4]}</span><b>${esc(word)}</b><small>${active?"🔊":esc(rest[n-1])}</small></button>`).join("")}</div></section>`; }
  ...
  function numbersVisualHTML(t) {
    ...
    const table = (title, rows) =>
      `<article class="a1-numbers-table"><h3>${esc(title)}</h3><div>${rows
        .map(([left, right]) => `<p><b>${esc(left)}</b><span>${esc(right)}</span></p>`)
        .join("")}</div></article>`;
    return `<div class="a1-numbers-grid">${table(t.rules?.[0]?.[0] || "", layout.zero20 || [])}${table(
      t.rules?.[1]?.[0] || "",
      layout.tens || [],
    )}${table(t.rules?.[2]?.[0] || "", layout.hundreds || [])}</div>`;
  }
  ...
  function runVocabularyDrill() {
    if (lesson?.activeRecall) return runActiveRecallDrill();
    remember("practice"); const r=root(), cards=shuffle(lesson.flashcards||[]), n={ru:1,uk:2,en:3,es:4}[lang()]||1; let i=0,score=0;
    function show(){if(i>=cards.length){practiceScore=score;return runStage("trainer",8)}const forward=i<5,card=cards[i],prompt=forward?card[0]:card[n],answer=forward?card[n]:card[0],opts=shuffle([answer,...shuffle(cards.filter(x=>x!==card).map(x=>forward?x[n]:x[0])).slice(0,2)]);r.innerHTML=`<div class="a1-lesson">${stepper("practice")}<div class="a1-kicker">A1 · ${esc((lesson.i18n?.[lang()]||lesson.i18n?.ru).practiceTitle)}</div><div class="a1-question-card"><div class="a1-counter">${i+1} / ${cards.length}</div><span class="a1-vocab-drill-icon">${card[5]}</span><button type="button" class="a1-audio" id="a1VocabListen">🔊 ${esc(U().listen)}</button><h2>${esc(prompt)}</h2><p>${esc(forward?({ru:"Выбери перевод",uk:"Обери переклад",en:"Choose the translation",es:"Elige la traducción"}[lang()]||""):({ru:"Выбери испанское слово",uk:"Обери іспанське слово",en:"Choose the Spanish word",es:"Elige la palabra en español"}[lang()]||""))}</p><div class="a1-options">${opts.map((o,k)=>`<button class="a1-option" data-vocab="${k}">${esc(o)}</button>`).join("")}</div><p id="a1Feedback"></p></div></div>`;document.getElementById("a1VocabListen")?.addEventListener("click",()=>speak(card[0]));r.querySelectorAll("[data-vocab]").forEach(b=>b.onclick=()=>{const ok=opts[+b.dataset.vocab]===answer;if(ok)score++;else weak.vocabulary=(weak.vocabulary||0)+1;document.getElementById("a1Feedback").textContent=ok?`✅ ${U().correct}`:`❌ ${U().wrong}: ${answer}`;r.querySelectorAll("[data-vocab]").forEach(x=>x.disabled=true);setTimeout(()=>{i++;show()},650)})}show()
  }
  function runActiveRecallDrill() {
    remember("practice");
    const r=root(), cards=shuffle(lesson.flashcards||[]).slice(0,12), n={ru:1,uk:2,en:3,es:4}[lang()]||1;
    const queue=shuffle(cards.flatMap(card=>[{card,forward:true,first:true},{card,forward:false,first:true}]));
    const initialTotal=queue.length; let shown=0,firstCorrect=0,retries=0;
    const text={ru:{toTranslation:"Выбери перевод",toSpanish:"Выбери испанское слово",retry:"Повторим это слово ещё раз через несколько карточек.",done:"Все слова закреплены."},uk:{toTranslation:"Обери переклад",toSpanish:"Обери іспанське слово",retry:"Повторимо це слово ще раз через кілька карток.",done:"Усі слова закріплено."},en:{toTranslation:"Choose the translation",toSpanish:"Choose the Spanish word",retry:"This word will return after a few cards.",done:"All words are consolidated."},es:{toTranslation:"Elige la traducción",toSpanish:"Elige la palabra en español",retry:"Esta palabra volverá después de unas tarjetas.",done:"Todas las palabras están consolidadas."}}[lang()]||{};
    function show(){
      if(!queue.length){practiceScore=Math.round((firstCorrect/Math.max(1,initialTotal))*6);return runStage("trainer",8)}
      const item=queue.shift(),card=item.card,answer=item.forward?card[n]:card[0],prompt=item.forward?card[0]:card[n];
      const options=shuffle([answer,...shuffle(cards.filter(x=>x!==card).map(x=>item.forward?x[n]:x[0])).slice(0,2)]);
      shown++;
      r.innerHTML=`<div class="a1-lesson">${stepper("practice")}<div class="a1-kicker">A1 · ${esc((lesson.i18n?.[lang()]||lesson.i18n?.ru).practiceTitle)}</div><div class="a1-question-card"><div class="a1-counter">${shown} / ${initialTotal}${retries?` · 🔁 ${retries}`:""}</div><span class="a1-vocab-drill-icon">${card[5]}</span><button type="button" class="a1-audio" id="a1VocabListen">🔊 ${esc(U().listen)}</button><h2>${esc(prompt)}</h2><p>${esc(item.forward?text.toTranslation:text.toSpanish)}</p><div class="a1-options">${options.map((o,k)=>`<button class="a1-option" data-vocab="${k}">${esc(o)}</button>`).join("")}</div><p id="a1Feedback" aria-live="polite"></p></div></div>`;
      document.getElementById("a1VocabListen")?.addEventListener("click",()=>speak(card[0]));
      r.querySelectorAll("[data-vocab]").forEach(b=>b.onclick=()=>{const ok=options[+b.dataset.vocab]===answer;if(ok&&item.first)firstCorrect++;if(!ok){weak.vocabulary=(weak.vocabulary||0)+1;queue.splice(Math.min(3,queue.length),0,{...item,first:false});retries++}document.getElementById("a1Feedback").textContent=ok?`✅ ${U().correct}: ${answer}`:`❌ ${U().wrong}: ${answer} · ${text.retry}`;r.querySelectorAll("[data-vocab]").forEach(x=>x.disabled=true);setTimeout(show,ok?650:1100)})
    }
    show();
  }
  ...
  function runStage(kind, count) {
    ...
    function show() {
      ...
      const audio = q.audio
        ...
        : "";
      const taskHint = q.speakOut
        ? ({ru:"Прослушайте реплику, выберите ответ и произнесите весь диалог вслух.",uk:"Прослухайте репліку, оберіть відповідь і промовте весь діалог уголос.",en:"Listen to the line, choose a reply and say the whole dialogue aloud.",es:"Escucha la frase, elige una respuesta y di todo el diálogo en voz alta."})[lang()]
        : ({ru:"Выберите один правильный вариант.",uk:"Оберіть один правильний варіант.",en:"Choose one correct answer.",es:"Elige una respuesta correcta."})[lang()];
      r.innerHTML = `<div class="a1-lesson">${stepper(kind)}<div class="a1-kicker">A1 · ${esc(title)}</div><div class="a1-question-card"><div class="a1-counter">${i + 1} / ${items.length}</div>${audio}<h2>${esc(q.prompt?.[lang()] || q.prompt?.ru || q.prompt || "")}</h2><p class="a1-task-hint">${esc(taskHint||"")}</p><div class="a1-options">${opts.map((o, n) => `<button type="button" class="a1-option" data-option-index="${n}">${esc(o)}</button>`).join("")}</div><p id="a1Feedback"></p></div></div>`;
