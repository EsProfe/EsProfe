...
(function () {
  ...
  function cityVisualHTML() {
    ...
    if (lesson?.flashcards) { const cards=lesson.flashcards||[], n={ru:1,uk:2,en:3,es:4}[lang()]||1,stair='<svg width="30" height="30" viewBox="0 0 48 48" aria-label="stairs"><path d="M5 40h10V30h10V20h10V10h8" fill="none" stroke="#1b55c9" stroke-width="5"/></svg>'; return `<section class="a1-vocab-cards"><h3>🧠 ${esc(({ru:"Карточки: нажми и вспомни перевод",uk:"Картки: натисни й згадай переклад",en:"Flashcards: tap and recall the translation",es:"Tarjetas: pulsa y recuerda la traducción"})[lang()]||"")}</h3><div>${cards.map(([word,...rest])=>`<button type="button" class="a1-vocab-card" data-card><span>${word==="la escalera"?stair:rest[4]}</span><b>${esc(word)}</b><small>${esc(rest[n-1])}</small></button>`).join("")}</div></section>`; }
  ...
  function runVocabularyDrill() {
    remember("practice"); const r=root(), cards=shuffle(lesson.flashcards||[]).slice(0,10), n={ru:1,uk:2,en:3,es:4}[lang()]||1; let i=0,score=0;
