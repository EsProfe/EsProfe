...
(function(){
  ...
  function languageOptionsForCountries(data){
    ...
    if(q){
      ...
      q.answers={
        ru:"со строчной буквы / con minúscula",
  ...
  function addHundredsGenderRule(data){
    ...
    Object.entries(additions).forEach(([code,text])=>{
      const rule=lesson.i18n?.[code]?.rules?.[2];
      if(rule && !rule[1].includes("doscientas páginas")) rule[1]+=text;
    });
    return data;
  }
  ...
  window.fetch=async function(input,init){
    ...
    try{
      ...
      console.error("A1 data normalization failed",e);
      return response;
    }
  };
})();