...
function recordAnswerProgress(verb,person,isCorrect){if(document.documentElement.dataset.learningMode==="course"){if(window.progressRecordAnswer)window.progressRecordAnswer(verb,person,isCorrect);}else if(document.documentElement.dataset.learningMode==="free"){window.EsProfeSessionProgress?.record(isCorrect);}}
...
function checkAnswer(){if(verbs.length===0)return;const verb=verbs[currentVerb];const input=document.getElementById("answer");const result=document.getElementById("result");const userAnswer=input.value.trim().toLowerCase();const correctAnswer=verb.present[currentPerson].toLowerCase();if(userAnswer===""){result.textContent=translations.emptyAnswer;return;}if(userAnswer===correctAnswer){correctAnswers++;removeMistake(verb,currentPerson);result.dataset.type="correct";result.dataset.answer="";result.textContent=`✅ ${translations.correct}`;recordAnswerProgress(verb,currentPerson,true);}else{wrongAnswers++;rememberMistake(verb,currentPerson);result.dataset.type="incorrect";result.dataset.answer=verb.present[currentPerson];result.textContent=`❌ ${translations.incorrect} ${translations.correctAnswer} ${verb.present[currentPerson]}`;recordAnswerProgress(verb,currentPerson,false);}updateStatus();updateMistakeButton();}
function updateStatus(){const status=document.getElementById("status");if(!status)return;status.textContent=`${translations.verbs} ${verbs.length} | ${translations.correctAnswers} ${correctAnswers} | ${translations.wrongAnswers} ${wrongAnswers}`;updateMistakeButton();if(document.documentElement.dataset.learningMode==="course"&&window.renderProgress)window.renderProgress();}
...
function checkTestAnswer(){if(!testActive)return;const input=document.getElementById("testAnswer");const result=document.getElementById("testResult");const userAnswer=input.value.trim().toLowerCase();if(userAnswer===""){result.textContent=translations.emptyAnswer;return;}const verb=verbs[currentVerb];const correctAnswer=verb.present[currentPerson].toLowerCase();if(userAnswer===correctAnswer){testCorrect++;result.dataset.type="correct";result.dataset.answer="";result.textContent=`✅ ${translations.correct}`;recordAnswerProgress(verb,currentPerson,true);}else{testWrong++;rememberMistake(verb,currentPerson);result.dataset.type="incorrect";result.dataset.answer=verb.present[currentPerson];result.textContent=`❌ ${translations.incorrect} ${translations.correctAnswer} ${verb.present[currentPerson]}`;recordAnswerProgress(verb,currentPerson,false);}testQuestion++;setTimeout(nextTestQuestion,800);}
function finishTest(){testActive=false;document.getElementById("testMode").style.display="none";document.getElementById("testResultScreen").style.display="block";document.getElementById("restartTest").textContent=translations.testAgain;document.getElementById("backToTraining").textContent=translations.backToTraining;const percent=Math.round((testCorrect/TEST_LENGTH)*100);if(document.documentElement.dataset.learningMode==="course"&&window.progressRecordTest)window.progressRecordTest(percent);document.getElementById("testFinished").textContent=translations.testFinished;document.getElementById("testScore").textContent=`${testCorrect} / ${TEST_LENGTH}`;const testPercent=document.getElementById("testPercent");testPercent.dataset.percent=percent;testPercent.textContent=`${translations.result} ${percent}%`;}
